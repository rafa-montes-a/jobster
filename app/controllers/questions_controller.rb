class QuestionsController < ApplicationController
  ...
  def create
    ...
    the_question.prompt = "Spend no more than 25 seconds answering this: I am applying to an " + the_question.job.role.to_s + "role at "  + the_question.job.firm_name.to_s + ". Answer this question: " + the_question.desc.to_s + ". Acting as an expert in the company and the role. You can use the following job description to guide your answer: " + the_question.job.job_desc.to_s
    ...
    if the_question.valid?
      the_question.save
      redirect_to("/jobs/#{ the_question.job_id }", { :notice => "Question created successfully." })
    else
      redirect_to("/jobs/#{ the_question.job_id }", { :alert => the_question.errors.full_messages.to_sentence })
    end
  end
